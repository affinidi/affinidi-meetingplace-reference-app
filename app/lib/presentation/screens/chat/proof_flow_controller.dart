...
final proofFlowControllerProvider = StateNotifierProvider.family<
    ProofFlowController, ProofFlowState, String>((ref, contactId) {
...
class ProofFlowController extends StateNotifier<ProofFlowState> {
  ...
  void dismissBanner() {
    ...
    final chatController =
        ref.read(chatScreenControllerProvider(contactId).notifier);
  ...
  /// Search for Liveness VC in wallet (always returns false for fresh generation)
  ...
  Future<void> generateAndSendProof({String? holderDid}) async {
    _logger.info('Starting proof generation for contact: $contactId', name: _logKey);
    ...
    try {
      ...
      _logger.info('Proof generated in ${proofResult.generationTimeMs}ms', name: _logKey);
      ...
      _logger.error('Failed to generate proof: $e', name: _logKey, stackTrace: st);
      state = state.copyWith(
        isGeneratingProof: false,
  ...
  Future<void> verifyProof() async {
    _logger.info('Starting proof verification for contact: $contactId', name: _logKey);
    ...
    try {
      ...
      _logger.info('Verification result: ${verificationResult.isValid}', name: _logKey);
  ...
  void onProofReceived(Map<String, dynamic> proofData) {
    ...
    _logger.info('  Inserting "proof received" concierge message', name: _logKey);
    final chatController =
        ref.read(chatScreenControllerProvider(contactId).notifier);
