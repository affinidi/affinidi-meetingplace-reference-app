...
void main() {
  ...
  group('ControlPlaneService', () {
    ...
    test('does not throw on channel activity before inauguration', () async {
      ...
      await runZonedGuarded(() async {
        final scopedFakeSdk = FakeMeetingPlaceSDK(
          channels: {
            ...
            'did:key:other-party': _channelWithStatus(ChannelStatus.approved),
          },
        );
        final scopedContainer = ProviderContainer(
          overrides: [
            meetingPlaceSdkProvider.overrideWith((ref) async => scopedFakeSdk),
            pushNotificationsHandlerProvider.overrideWith(
              _FakePushNotificationsHandler.new,
            ),
            networkConnectivityServiceProvider.overrideWith(
        ...
        final subscription = scopedControlPlaneService.onChannelActivity.listen(
          (_) {
    ...
    test('emits channel activity when stale payload is reconciled', () async {
      var emittedActivityCount = 0;

      final staleEventChannel = _channelWithStatus(
        ChannelStatus.approved,
        permanentChannelDid: 'did:key:channel-1',
        otherPartyPermanentChannelDid: 'did:key:other-party-1',
      );
      final sdkChannel = _channelWithStatus(
        ChannelStatus.inaugurated,
        permanentChannelDid: 'did:key:channel-1',
        otherPartyPermanentChannelDid: 'did:key:other-party-1',
      );
      ...
      final scopedFakeSdk = FakeMeetingPlaceSDK(
        channels: {
          'did:key:channel-1': sdkChannel,
          'did:key:other-party-1': sdkChannel,
        },
      );
      final scopedContainer = ProviderContainer(
        overrides: [
      ...
      scopedFakeSdk.simulateChannelActivity(staleEventChannel);
      await Future<void>.delayed(const Duration(milliseconds: 1));
...
Channel _channelWithStatus(
  ...
  return Channel(
    permanentChannelDid: permanentChannelDid ?? contact.channelDid!,
